...
The current stemmatic network is computed from agreement rates across all variant loci. Once you have annotated the loci, export your JSON and the agreement rates can be recomputed using **only the significant loci**. See the companion Python tool ([`mahadurot_collator.py`](https://github.com/rinahillmann/mahadurot-collation-editor/blob/main/scripts/mahadurot_collator.py)) for command-line processing and significant-only stemma generation.
